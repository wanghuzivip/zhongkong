	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="main_table">
		...
		<c:choose>
			<c:when test="${not empty departMents}">
				<c:forEach items="${departMents}" var="role" varStatus="vs">
				<tr class="main_info">
				<td>${vs.index+1}</td>
				<td id="roleNameTd${role.departId }">${role.departName }</td>
				<td><a href="javascript:editRole(${role.departId });">修改</a> | <a href="javascript:delUser(${role.departId });">删除</a> | <a href="javascript:editRights(${role.departId });">权限</a></td>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
